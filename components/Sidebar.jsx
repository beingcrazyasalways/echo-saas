...
export default function Sidebar({ currentEmotion, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <div className={`w-64 sm:w-72 h-screen backdrop-blur-xl bg-slate-900/50 border-r border-white/10 flex flex-col fixed left-0 top-0 lg:static lg:left-auto lg:inset-auto z-60 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-4 sm:p-6 space-y-2 pointer-events-auto">
          {menuItems.map((item) => {
            ...
            return (
              <button
                key={item.path}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 pointer-events-auto ${
                  isActive
